...
#98はPR #101で完了済み。#99（PR #102）が完了するまで#24はBlocked。現行の128件履歴だけで全disc履歴を表示しない。
...
workerが一次read結果を最大256区間へ集約する。停止/seekをまたいで保持し、TOC再受理でresetする。
...
field・flags・世代照合はメッセージ契約を正本とする。#24の描画は未実装。
