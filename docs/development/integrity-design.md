...
この文書は未実装の拡張候補を扱う。現在の要件・構成は[基本設計](../design/basic-design.md)、
利用可能な振る舞いは[機能設計](../design/functional-design.md)、処理契約は
[詳細設計](../design/detailed-design.md)を正とする。実装済み部分はそれらへ統合した。
現在の実機確認範囲は[検証状況](verification.md)を参照する。
設計基準cbf458eからの拡張検討であり、以下の候補型・数値・phaseは現行機能の保証ではない。
...
以下の拡張案だけを理由に既存reader、ALSA、CEC、metadataを再設計しない。
...
| 1b Observable presentation | NO DISC能力表示、technical statusの小さなrenderer、event受信 | 実装・自動試験済み。通常再生、再読み込み、再接続をbrowserで実機確認済み。kiosk化は別作業 |
| 2 Buffered Reader | 既存queueの容量/閾値設定、device I/O調停、速度設定と失敗fallback | 容量/閾値と直列化を実装し、通常CDで実機比較済み。速度設定、傷disc・長時間stall評価は未実装 |
...
現在の確認済み範囲と実測は[検証状況](verification.md)を参照する。
