...
ReadEvidence make_read_evidence(const ReadResult& result) {
    ...
    else if (backend_recovered) evidence.status = IntegrityReadStatus::recovered;
    else if (anomaly) evidence.status = IntegrityReadStatus::uncertain;
    else evidence.status = IntegrityReadStatus::clean;
    evidence.local_verification = backend_reported_verification(result.paranoia)
                                      ? LocalVerification::backend_reported
                                      : LocalVerification::single_read;
